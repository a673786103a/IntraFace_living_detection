...
//检测左眼
int detectEyeBinkLeft( cv::Mat &demo,const cv::Mat &pts ,float thres_ = 0.26)
{
	...
	{
		char buf_thres[30];
		sprintf_s(buf_thres, "Left eye threshold:%lf", ratio_eye);
		std::cout << "left eye close" << std::endl;
		putText(demo, "Left eye: colse", Point(50, 100), FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(0, 0, 255), 2);
		putText(demo, buf_thres, Point(50, 120), FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(0, 0, 255), 2);
		return 1;
	}
	//左眼开
	else {
		...
		putText(demo, "Left eye: open", Point(50, 100), FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(0, 0, 255), 2);
		putText(demo, buf_thres, Point(50, 120), FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(0, 0, 255), 2);
	}
...
//检测右眼
int detectEyeBinkRight(cv::Mat &demo, const cv::Mat &pts, float thres_ = 0.26)
{
	// eye norm left p1 19 p2 22
	// eye norm left p1 25 p2 28
	...
	if (ratio_eye < thres_)
	{
		//
		char buf_thres[30];
		sprintf_s(buf_thres, "Right eye threshold:%lf", ratio_eye);
		std::cout << "Right eye close" << std::endl;
		putText(demo, "Right eye: colse", Point(50, 140), FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(0, 0, 255), 2);
		putText(demo, buf_thres, Point(50, 160), FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(0, 0, 255), 2);

		return 1;
	}
	else {
		char buf_thres[30];
		sprintf_s(buf_thres, "Right eye thresold:%lf", ratio_eye);
		std::cout << "Right eye open" << std::endl;
		putText(demo, "Right eye: open", Point(50, 140), FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(0, 0, 255), 2);
		putText(demo, buf_thres, Point(50, 160), FONT_HERSHEY_COMPLEX_SMALL, 1, Scalar(0, 0, 255), 2);
	}
...
public:
	...
	void show(cv::Mat &frame)
	{
		...
		if(text_alive == success)
			putText(frame, text_alive, cv::Point(50, 40), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0, 255, 0), 1);
		else if(text_alive == failed)
			putText(frame, text_alive, cv::Point(50, 40), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0, 0, 255), 1);
		else
			putText(frame, text_alive, cv::Point(50, 40), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(255, 0, 0), 1);
	}
